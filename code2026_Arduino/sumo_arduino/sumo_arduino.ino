...
// ────────────────────────────────────────────
//  LEER BORDE (IR)
//  Doble lectura anti-ruido sin delay entre ellas. La direccion de
//  deteccion (si el borde da lecturas mas altas o mas bajas que el
//  umbral) se detecta sola durante la calibracion, asi que funciona
//  sin importar como se comporte tu sensor especifico.
// ────────────────────────────────────────────
...
String leerBorde() {
  ...
  bool det[4];
  for (int i = 0; i < 4; i++) {
    det[i] = esBorde(v1[i], i) && esBorde(v2[i], i);
  }
...
void calibracionPorPasos() {
  ...
  // Diagnostico: verifica que la calibracion se hizo en el orden correcto.
  // Si "blancoEsMenor" sale en "NO" para un sensor, revisa que en el
  // PASO 1 realmente estuviera sobre negro y en el PASO 2 sobre blanco.
  ...
  for (int i = 0; i < 4; i++) {
    ...
    Serial.print(" | blancoEsMenor: "); Serial.println(blancoEsMenor[i] ? "SI (normal)" : "NO (revisar orden de calibracion)");
  }
  Serial.println("------------------------------");
